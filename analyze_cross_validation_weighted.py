...
"""
交叉验证结果分析工具（weighted F1-score版本）
专门用于分析五折交叉验证的测试结果
"""
...
def analyze_fold_results(smell_type, fold_idx):
    """分析单个fold的结果"""
    file_path = f'results_weighted/{smell_type}_fold{fold_idx}_test_results.json'
...
def analyze_cross_validation(smell_type, selected_folds=None):
    ...
    print("🚀" + "="*60)
    print(f"📊 交叉验证结果分析（weighted F1-score版本）")
    ...
    output_file = f'performance_analysis_cross_validation_weighted_{smell_type}.json'
...
def main():
    ...
    print("🚀 交叉验证结果分析工具（weighted F1-score版本）")
    print("="*50)
    ...
    if fold_input.lower() == 'all' or fold_input == '':
        ...
        selected_folds = []
        for fold_idx in range(1, 6):  # 假设最多5个fold
            file_path = f'results_weighted/{selected_smell}_fold{fold_idx}_test_results.json'
            if os.path.exists(file_path):
                selected_folds.append(fold_idx)
