...
class EnhancedCodeSmellDetector:
    ...
    def _load_fusion_config(self, smell_type: str, fold_idx: int = None) -> Dict:
        ...
        """
        # 优先尝试加载包含fold信息的配置文件（weighted版本使用results_weighted目录）
        if fold_idx is not None:
            config_path = f"results_weighted/{smell_type}_fold{fold_idx}_config.json"
        ...
        config_path = f"results_weighted/{smell_type}_fusion_config.json"
