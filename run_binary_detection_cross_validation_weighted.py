...
from enhanced_detection_weighted import EnhancedCodeSmellDetector, load_test_data_for_smell
...
class CrossValidationLLMStrategy:
    ...
    def __init__(self, model_path, smell_type, fold_idx, use_dynamic_strategy=False, offline=False):
        ...
        # 五折交叉验证阈值管理
        self.thresholds_dir = "thresholds_weighted"
    ...
    def _setup_dynamic_thresholds(self):
        ...
        print(f"[OK] 设置Fold {self.fold_idx}的训练阈值: [{LLM_CONFIDENCE_THRESHOLD_LOW}, {LLM_CONFIDENCE_THRESHOLD_HIGH}]")
    ...
    def _load_or_calculate_thresholds(self):
        ...
        if os.path.exists(threshold_file):
            try:
                ...
                print(f"[OK] 加载Fold {self.fold_idx}的阈值: {thresholds}")
                return thresholds
            except Exception as e:
                print(f"[ERROR] 加载阈值文件失败: {e}")
        
        # 如果阈值文件不存在，计算最优阈值
        print(f"[INFO] 计算Fold {self.fold_idx}的最优阈值...")
        
        # 加载训练数据
        from enhanced_detection_weighted import load_test_data_for_smell
        train_data, all_data_dict = load_test_data_for_smell(
            self.smell_type, self.model_path, self.fold_idx, "training"
        )
        
        if not train_data:
            print("[ERROR] 无法加载训练数据，使用默认阈值")
        ...
        try:
            ...
            print(f"[OK] 保存Fold {self.fold_idx}的阈值: {optimal_thresholds}")
        except Exception as e:
            print(f"[ERROR] 保存阈值文件失败: {e}")
        ...
        if os.path.exists(cache_file):
            try:
                with open(cache_file, 'r', encoding='utf-8') as f:
                    cache = json.load(f)
                print(f"[OK] 加载LLM缓存: {len(cache)} 条记录")
                return cache
            except Exception as e:
                print(f"[ERROR] 加载LLM缓存失败: {e}")
    ...
    def _save_llm_cache(self):
        ...
        try:
            ...
            print(f"[OK] 保存LLM缓存: 新增{len(self.llm_cache)}条，合并后共{len(merged_cache)}条记录")
        except Exception as e:
            print(f"[ERROR] 保存LLM缓存失败: {e}")
    ...
    def _calculate_optimal_thresholds(self, train_data):
        ...
        for data in train_data:
            try:
                ...
                print(f"[ERROR] 处理训练样本失败: {e}")
                continue

        if not confidences:
            print("[ERROR] 无法获取训练集置信度，使用默认阈值")
            return {"low": 0.3, "high": 0.7}
        ...
        print(f"[INFO] Fold {self.fold_idx}使用15个等距区间方法计算阈值...")
        ...
        print(f"[OK] 全局最优阈值: {best_threshold:.2f} (F1={best_f1:.4f})")
        ...
        print(f"\n[INFO] 步骤2: 15个等距区间分析 ({min_conf:.4f} - {max_conf:.4f}):")
        ...
        if low_confidence_zones:
            ...
            if merged_low_zones:
                if len(merged_low_zones) > 1:
                    ...
                    print(f"\n[INFO] 识别出多个低置信区间，合并为整体范围: [{best_low:.4f}, {best_high:.4f})")
                    print(f"   包含的具体区间: {merged_low_zones}")
                else:
                    # 单个连续低置信区间
                    best_low = merged_low_zones[0][0]
                    best_high = merged_low_zones[0][1]
                    print(f"\n[INFO] 识别出低置信区间: [{best_low:.4f}, {best_high:.4f})")
            else:
                best_low = 0.3
                best_high = 0.7
                print("[INFO] 未识别出低置信区间，使用默认阈值")
        else:
            best_low = 0.3
            best_high = 0.7
            print("[INFO] 未识别出低置信区间，使用默认阈值")
        
        print(f"[INFO] Fold {self.fold_idx}最优阈值: [{best_low:.4f}, {best_high:.4f}]")
...
def run_single_fold(smell_type, fold_idx, model_path, use_dynamic_strategy=False, 
                   offline=False, show_examples=3, output_dir='results_weighted'):
    """运行单个fold的检测（包含训练集配置生成和测试集检测）"""
    print(f"\n{'='*60}")
    print(f"[START] 开始Fold {fold_idx}检测")
    print(f"[INFO] 坏味类型: {smell_type}")
    print(f"[INFO] 动态策略: {'启用' if use_dynamic_strategy else '传统框架'}")
    ...
    # 第一步：运行训练集检测以生成配置
    print(f"[INFO] Fold {fold_idx} - 运行训练集检测生成配置...")
    ...
    print(f"[INFO] Fold {fold_idx}: 使用策略 - {strategy_summary}")
    
    # 加载训练数据
    train_data, all_data_dict = load_test_data_for_smell(smell_type, model_path, fold_idx, "training")
    
    if not train_data:
        print(f"[ERROR] Fold {fold_idx}无法加载训练数据")
        return {}
    
    print(f"[INFO] Fold {fold_idx}训练数据: {len(train_data)}个样本")
    ...
    if os.path.exists(train_checkpoint_file):
        ...
        print(f"[LOAD] 加载训练集断点，已处理 {len(train_results)} 个样本")
    ...
    print(f"[LOAD] 处理新训练样本: {len(new_train_data)}个")
    ...
    for i, data in enumerate(new_train_data):
        ...
        if (i + 1) % 50 == 0:
            ...
            print(f"[SAVE] 已处理 {i+1}/{len(new_train_data)} 训练样本，保存断点...")
    
    print(f"[OK] Fold {fold_idx} 训练集批量预测完成，处理样本数: {len(train_results)}")
    ...
    print(f"[OK] Fold {fold_idx}训练集结果已保存: {train_result_file}")
    
    # 保存训练集处理过程中生成的LLM缓存
    if hasattr(strategy, '_save_llm_cache'):
        strategy._save_llm_cache()
        print(f"[SAVE] Fold {fold_idx}训练集LLM缓存已保存")
    
    # 第二步：基于训练集结果生成配置
    print(f"[INFO] Fold {fold_idx} - 生成测试集配置...")
    
    # 导入配置生成器
    from generate_config_weighted import ConfigGenerator
    ...
    if config_success:
        print(f"[OK] Fold {fold_idx}配置生成成功: {config_file}")
        ...
        try:
            ...
            print(f"[OK] Fold {fold_idx}配置加载成功")
            ...
            print(f"[INFO] Fold {fold_idx}策略已更新: {len(strategy.optimal_thresholds)}个风险区间")
            
        except Exception as e:
            print(f"[ERROR] Fold {fold_idx}配置加载失败: {e}")
    else:
        print(f"[ERROR] Fold {fold_idx}配置生成失败，使用默认配置")
    
    # 第三步：运行测试集检测 - 添加断点保存功能
    print(f"[INFO] Fold {fold_idx} - 运行测试集检测...")
    
    # 加载测试数据
    test_data, _ = load_test_data_for_smell(smell_type, model_path, fold_idx, "testing")
    
    if not test_data:
        print(f"[ERROR] Fold {fold_idx}无法加载测试数据")
        return {}
    ...
    if os.path.exists(test_checkpoint_file):
        ...
        print(f"[LOAD] 加载测试集断点，已处理 {len(test_results)} 个样本")
    ...
    print(f"[LOAD] 处理新测试样本: {len(new_test_data)}个")
    ...
    for i, data in enumerate(new_test_data):
        ...
        if (i + 1) % 50 == 0:
            ...
            print(f"[SAVE] 已处理 {i+1}/{len(new_test_data)} 测试样本，保存断点...")
    
    print(f"[OK] Fold {fold_idx} 测试集批量预测完成，处理样本数: {len(test_results)}")

    # 使用固定阈值0.5计算最终预测
    print(f"\n[INFO] 使用固定阈值0.5计算最终预测...")
    ...
    print(f"\n[OK] Fold {fold_idx} 完成，测试集结果保存至: {test_result_file}")
...
def main():
    ...
    parser.add_argument('--output-dir', default='results_weighted',
                       help='输出目录')
    ...
    print("[START] 启动五折交叉验证LLM辅助验证脚本（weighted F1-score版本）")
    print("[INFO] 使用固定随机种子666确保可复现性")
    print(f"[INFO] 坏味类型: {args.smell_type}")
    print(f"[INFO] 动态策略: {'启用' if args.dynamic_strategy else '传统框架'}")
    print(f"[INFO] 总fold数: {args.total_folds}")
    ...
    for fold_idx in range(1, args.total_folds + 1):
        ...
        if not os.path.exists(model_path):
            print(f"[ERROR] Fold {fold_idx}: 模型文件不存在: {model_path}")
            continue
        ...
        try:
            ...
            print(f"[ERROR] Fold {fold_idx} 运行失败: {e}")
            import traceback
            traceback.print_exc()
    
    print("\n[OK] 五折交叉验证完成!")
    print("[INFO] 请使用独立的 analyze_cross_validation_weighted.py 脚本进行结果分析")

if __name__ == "__main__":
    # 使用示例
    if len(sys.argv) == 1:
        print("=" * 60)
        print("[START] 五折交叉验证LLM辅助验证脚本（weighted F1-score版本）")
        print("=" * 60)
        print("\n[INFO] 使用示例:")
        print("python run_binary_detection_cross_validation_weighted.py --smell-type feature_envy --model-base-path model_weighted/MyDNN_fusion_feature_envy_fold1_binary.pth")
        print("python run_binary_detection_cross_validation_weighted.py --smell-type blob --model-base-path model_weighted/MyDNN_fusion_blob_fold1_binary.pth --dynamic-strategy")
        print("python run_binary_detection_cross_validation_weighted.py --smell-type long_method --model-base-path model_weighted/MyDNN_fusion_long_method_fold1_binary.pth --offline")
        print("python run_binary_detection_cross_validation_weighted.py --smell-type data_class --model-base-path model_weighted/MyDNN_fusion_data_class_fold1_binary.pth")
    else:
        main()