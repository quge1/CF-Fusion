...
def find_best_threshold(y_true, y_prob, metric='f1'):
    ...
    for threshold in thresholds:
        ...
        if metric == 'f1':
            value = f1_score(y_true, y_pred, average='binary', zero_division=0)
        elif metric == 'precision':
            value = precision_score(y_true, y_pred, average='binary', zero_division=0)
        elif metric == 'recall':
            value = recall_score(y_true, y_pred, average='binary', zero_division=0)
...
def test(testlist, model_index, model=None, use_best_threshold=False):
    ...
    # 计算p, r, f1, acc
    p = precision_score(y_test, y_pred_label, average='binary', zero_division='warn')
    r = recall_score(y_test, y_pred_label, average='binary')
...
def train_single_fold(model, train_label_list, test_label_list, fold_idx, test_result):
    ...
    best_model_path = os.path.join(model_dir, f"{curModel.__name__}_{smell}_fold{fold_idx}_binary.pth")
    checkpoint_path = os.path.join(model_dir, f"{curModel.__name__}_{smell}_fold{fold_idx}_binary_checkpoint.pth")
